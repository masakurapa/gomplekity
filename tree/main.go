...
func generateTreeSVG(width, height int, colorRatio ColorRatio) string {
	...
	// Grassland with depth and dimension
	...
	// Add wind-blown grass on top of flat base
	...
	for i := 0; i < 200; i++ {
		...
		// Starting point at ground
		startX := x
		startY := float64(height - 50)
		
		// Control point (middle of grass with wind bend)
		controlX := x + bendAmount*0.6
		controlY := startY - grassHeight*0.7
		
		// End point (tip of grass)
		endX := x + bendAmount
		endY := startY - grassHeight
		
		// Random color for natural variation
	...
	for i := 0; i < 50; i++ {
		...
		for j := 0; j < clusterSize; j++ {
			...
			// Cluster grass with similar wind direction
